...
def test_full_size_inference():
    ...
    segmentation_weightsfile = join('weights', 'weights_segmentation_all.pickle')
    print(
        'loading weights for the segmentation network from %s'
        % (segmentation_weightsfile)
    )
    model.load_weights(layer_dict['seg_out'], segmentation_weightsfile)
    ...
    img_chw = (img_hwc / 255.0).transpose(2, 0, 1).astype(np.float32)
    img_bchw = img_chw.reshape(-1, img_chw.shape[0], img_chw.shape[1], img_chw.shape[2])
    seg_bchw = infer_func(img_bchw)
    seg_hwc = seg_bchw[0].transpose(1, 2, 0)
    print('seg.shape = %r' % (seg_hwc.shape,))
    print('seg.min() = %.2f, seg.max() = %.2f' % (seg_hwc.min(), seg_hwc.max()))
